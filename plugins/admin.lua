...
local function logrem(msg)
	...
	save_data(_config.moderation.data, data)
	local text = 'سوپرگروه از لوگ حذف شد'
	reply_msg(msg.id,text,ok_cb,false)
	return
end
...
local function run(msg,matches)
 ...
 if matches[1] == "mycontact" and is_sudo(msg) then
	...
	if matches[1] == 'آپدیت' then
		...
		reload_plugins(true)
		post_msg(receiver, "ربات اپدیت شد", ok_cb, false)
		return "ربات اپدیت شد"
	end
	--[[*For Debug*
	...
	if matches[1] == 'اضافه کردن لوگ' and not matches[2] then
		if is_log_group(msg) then
			return "سوپرگروه از قبل لوگ بوده"
		end
		print("سوپر گروه به عنوان لوگ"..msg.to.title.."("..msg.to.id..") اد شد")
	...
	if matches[1] == 'حذف لوگ' and not matches[2] then
		if not is_log_group(msg) then
			return "سوپرگروه لوگ نبوده است"
		end
		print("سوپرگروه به عنوان لوگ"..msg.to.title.."("..msg.to.id..") حذف شد")
		savelog(msg.to.id, name_log.." ["..msg.from.id.."] added Log_SuperGroup")
		logrem(msg)
	end
    return
