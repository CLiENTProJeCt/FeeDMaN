...
local support_id = msg.from.id
 if matches[1]:lower() == 'ایدی' and msg.to.type == "chat" or msg.to.type == "user" then
  ...
  if matches[1]:lower() == 'منع' then-- /ban
    ...
    elseif string.match(matches[2], '^%d+$') then
        ...
        if not is_admin1(msg) and is_momod2(matches[2], msg.to.id) then
          	return "شما نمیتوانید ادمین هارا منع کنید."
        end
        if tonumber(matches[2]) == tonumber(msg.from.id) then
          	return "شما نمیتوانید خود را منع کنید"
        end
        local print_name = user_print_name(msg.from):gsub("‮", "")
	    local name = print_name:gsub("_", "")
		local receiver = get_receiver(msg)
  ...
  if matches[1]:lower() == 'رفع ممنوعیت' then -- /unban
    if type(msg.reply_id)~="nil" and is_momod(msg) then
      ...
      if string.match(targetuser, '^%d+$') then
        	...
        	return 'یوزر'..user_id..' رفع منع شد'
      else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unban',
...
if matches[1]:lower() == 'اخراج' then
  ...
  if matches[1]:lower() == 'منع جهانی' and is_admin1(msg) then -- Global ban
      ...
      if string.match(targetuser, '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	banall_user(targetuser)
       		return 'یوزر ['..user_id..' ] منع جهانی شد.'
     else
	local cbres_extra = {
		chat_id = msg.to.id,
		get_cmd = 'banall',
  ...
  if matches[1]:lower() == 'رفع منع جهانی' then -- Global unban
    local user_id = matches[2]
    local chat_id = msg.to.id
      if string.match(matches[2], '^%d+$') then
        if tonumber(matches[2]) == tonumber(our_id) then
        	...
        	return 'یوزر['..user_id..' ] رفع منع جهانی شد'
    else
		local cbres_extra = {
			chat_id = msg.to.id,
			get_cmd = 'unbanall',
  ...
  if matches[1]:lower() == "لیست منع جهانی" then -- Global ban list
    return banall_list()
  end
end
