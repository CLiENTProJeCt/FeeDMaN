...
local function run(msg, matches)
  if matches[1]:lower() == 'فید' then -- Put everything you like :)
    local about = _config.about_text
    local name = user_print_name(msg.from)
    savelog(msg.to.id, name.." ["..msg.from.id.."] used /feed ")
    return about
  end 
  if matches[1]:lower() == "لیست امار" then
    if not is_momod(msg) then
              return "برای ادمین های ربات است"
    end
  ...
  if matches[1]:lower() == "امار" then
    ...
    if matches[2] == "فید" then -- Put everything you like :)
