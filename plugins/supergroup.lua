...
local text = "افراد گروه "..cb_extra.receiver
local i = 1
for k,v in pairsByKeys(result) do
...
local function lock_group_spam(msg, data, target)
  ...
  if not is_owner(msg) then
    return "ادمین دسترسی دارد!"
  end
  local group_spam_lock = data[tostring(target)]['settings']['lock_spam']
  if group_spam_lock == 'yes' then
    return 'قفل اسپم فعال بوده است'
  else
...
local function unlock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return 'قفل فلود فعال نبوده است'
  else
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'قفل عربی غیرفعال بوده است'
  else
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return 'قفل ممبر غیرفعال بوده است'
  else
    data[tostring(target)]['settings']['lock_member'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل ممبر غیرفعال شد'
  end
end
...
local function lock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'yes' then
    return 'قفل سیستم فعال بوده است'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'قفل سیستم فعال شد'
  end
end
...
local function unlock_group_tgservice(msg, data, target)
  ...
  if group_tgservice_lock == 'no' then
    return 'قفل سیستم فعال نبوده سات'
  else
    data[tostring(target)]['settings']['lock_tgservice'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل سیستم غیرفعال شد'
  end
end
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return 'قفل استیکر غیرفعال بوده است'
  else
...
local function lock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'yes' then
    return 'قفل شماره فعال بوده است'
  else
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'no' then
    return 'قفل شماره از قبل غیرفعال بوده است.'
  else
    data[tostring(target)]['settings']['lock_contacts'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل شماره غیرفعال شد'
  end
end
...
local function enable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'yes' then
    return 'تنظیمات سخت گیرانه فعال بوده است.'
  else
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return 'تنظیمات سخ گیرانه فعال نبوده است'
  else
...
local function get_rules(msg, data)
  ...
  if not data[tostring(msg.to.id)][data_cat] then
    return 'قوانین از قبل تعیین نشده است.'
  end
...
local function set_public_membermod(msg, data, target)
  ...
  if group_public_lock == 'yes' then
    return 'گروه در لیست پابلیک بوده است'
  else
...
local function unset_public_membermod(msg, data, target)
  ...
  if group_public_lock == 'no' then
    return 'گروه در لیست پابلیک نیست'
  else
    data[tostring(target)]['settings']['public'] = 'no'
	data[tostring(target)]['long_id'] = msg.to.long_id
    save_data(_config.moderation.data, data)
    return 'وضعیت گروه: پابلیک نیست'
  end
end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'اضافه' and not matches[2] then
			...
			if is_super_group(msg) then
				return reply_msg(msg.id, 'سوپر گروه از قبل ضافه شده بوده است', ok_cb, false)
			end
		...
		if matches[1] == "مالک" then
			...
			if not group_owner then
				return "هیچ اونری تایین نشده!"
			end
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] used /owner")
			return "اونر گروه ["..group_owner..']'
		end
		...
		if matches[1] == 'قراردادن لینک' and is_owner(msg) then
			...
			save_data(_config.moderation.data, data)
			return 'لطفا لینک خود را ارسال کنید.'
		end
		...
		if matches[1] == 'لینک' then
			...
			if not group_link then
				return "خطلا!!!\n اگر من در گروه اد شده ام دستور قراردادن لینک بزنید."
			end
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested group link ["..group_link.."]")
			return "لینک گروه:\n"..group_link
		end

		if matches[1] == "وارد کردن" and is_sudo(msg) then
			local cbres_extra = {
		...
		--[[if matches[1] == 'kick' and is_momod(msg) then
			...
			elseif matches[1] == 'تنزیل ادمین' and matches[2] and string.match(matches[2], '^%d+$') then
			--[[]	local receiver = get_receiver(msg)
				...
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'تنزیل ادمین' and matches[2] and not string.match(matches[2], '^%d+$') then
				--[[local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'setadmin'
				}
		...
		if matches[1] == 'مالک' and is_owner(msg) then
			...
			elseif matches[1] == 'مالک' and matches[2] and string.match(matches[2], '^%d+$') then
		--[[	local group_owner = data[tostring(msg.to.id)]['set_owner']
				if group_owner then
					local receiver = get_receiver(msg)
				...
				local user_id = matches[2]
				channel_get_users (receiver, in_channel_cb, {get_cmd=get_cmd, receiver=receiver, msg=msg, user_id=user_id})
			elseif matches[1] == 'مالک' and matches[2] and not string.match(matches[2], '^%d+$') then
		...
		if matches[1] == 'تنزیل' then
		  if not is_momod(msg) then
				return
			end
			if not is_owner(msg) then
				return "فقط مدیر دسترسی دارد"
			end
			if type(msg.reply_id) ~= "nil" then
				local cbreply_extra = {
					get_cmd = 'promote',
			...
			elseif matches[1] == 'تنزیل' and matches[2] and string.match(matches[2], '^%d+$') then
				local receiver = get_receiver(msg)
				local user_id = "user#id"..matches[2]
				local get_cmd = 'promote'
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] promoted user#id"..matches[2])
				user_info(user_id, cb_user_info, {receiver = receiver, get_cmd = get_cmd})
			elseif matches[1] == 'تنزیل' and matches[2] and not string.match(matches[2], '^%d+$') then
				local cbres_extra = {
					channel = get_receiver(msg),
					get_cmd = 'promote',
				}
		...
		if matches[1] == "قراردادن یوزرنیم" and is_admin1(msg) then
		...
		if matches[1] == 'ست فلود' then
			...
			if tonumber(matches[2]) < 5 or tonumber(matches[2]) > 85 then
				return "Wrong number,range is [5-85]"
			end
			local flood_max = matches[2]
			data[tostring(msg.to.id)]['settings']['flood_msg_max'] = flood_max
			save_data(_config.moderation.data, data)
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] set flood to ["..matches[2].."]")
			return 'فلود تعیین شده: '..matches[2]
		end
		if matches[1] == 'پابلیک' and is_momod(msg) then
			local target = msg.to.id
			if matches[2] == 'بله' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] وضعیت سوپرگروه: پابلیک")
				return set_public_membermod(msg, data, target)
			end
			if matches[2] == 'خیر' then
				savelog(msg.to.id, name_log.." ["..msg.from.id.."] وضعیت سوپرگروه: پابلیک نیست")
				return unset_public_membermod(msg, data, target)
			end
		end

		if matches[1] == 'بیصداکردن' and is_owner(msg) then
			local chat_id = msg.to.id
		...
		if matches[1] == 'باصداکردن' and is_momod(msg) then
			local chat_id = msg.to.id
			if matches[2] == 'صدا' then
			local msg_type = 'Audio'
